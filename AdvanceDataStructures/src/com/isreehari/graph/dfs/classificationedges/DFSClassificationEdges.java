 ...
 */
package com.isreehari.graph.dfs.classificationedges;
 ...
 * @author sinukoll
 */
public class DFSClassificationEdges {
    ...
    public static void main(String... args){
        ...
        matrix.dfsTraversal();
        System.out.println();
        System.out.print("Does Grap has cycle: ");
        System.out.print(matrix.isCyclic());
        System.out.println();
    ...
    private void dfs(int currentVertex){
        ...
        for(int i = 0; i < this.numberVerticies; i++){
            if(this.isAdjacent(currentVertex, i)){
                    if( this.vertexList[i].state == INITIAL){
                        System.out.println("Tree Edge - ("+ this.vertexList[currentVertex]+","+ this.vertexList[i] +")");
                        this.dfs(i);
                    }else if( this.vertexList[i].state == VISITED){
                        System.out.println("Back Edge - ("+ this.vertexList[currentVertex]+","+ this.vertexList[i] +")");                        
                    }else if( this.vertexList[currentVertex].discoveryTime < this.vertexList[i].discoveryTime){
                        System.out.println("Forward Edge - ("+ this.vertexList[currentVertex]+","+ this.vertexList[i] +")");                        
                    }else{
                        System.out.println("Cross Edge - ("+ this.vertexList[currentVertex]+","+ this.vertexList[i] +")");                        
                    }
    ...
    private void dfsc(int currentVertex){
        this.vertexList[currentVertex].state = VISITED;
        for(int i = 0; i < this.numberVerticies; i++){
            if(this.isAdjacent(currentVertex, i)){
                    if( this.vertexList[i].state == INITIAL){                        
                        this.dfsc(i);
                    }else if( this.vertexList[i].state == VISITED){ // check for back edge 
                       hasCycle = true;                       
                    }                    
            }                
        }        
